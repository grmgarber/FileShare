...
class User < ActiveRecord::Base
  ...
  has_many :uploads
  has_many :grants # these are grants given TO the user.
  # Do not care about those given BY the user
  ...
  def self.potential_grantee_emails(user_id, upload_id, term: '', limit: 30)
    upload_grantee_ids = grantee_user_ids(upload_id)
    q = User.where.not(id: user_id)
    q = q.where.not(id: upload_grantee_ids) unless upload_grantee_ids.empty?
